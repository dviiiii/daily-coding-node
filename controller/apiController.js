...
const apitypeModel = require('../model/api/apitype');
const apiItemModel = require('../model/api/apiitem');

const apiController = {
    addApiType (req, res, next) {
        apitypeModel.find({apiTypeName: req.body.apiTypeName}, function (err, docs) {
            if(err) {
                res.json({
                    state: 0
                });
                ...
                if(docs.length === 0) {
                    let parmas = req.body;
                    apitypeModel.create(parmas, function (err, docs) {
                        if(err) {
                            res.json({
                                state: 0
                            });
